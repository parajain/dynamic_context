...
ALLEN_ES_LINKS = 'joint_entity_links' # these are linking obtained by running joint link script
ALLEN_TYPE_LINKS='joint_type_links'
...
parser = argparse.ArgumentParser(description='Pre-compute candidate expansion sub-graphs')
...
parser.add_argument('--jointNEL_folder', help='.')
...
print(args)
ROOT_PATH=args.read_folder
DST_ROOT_PATH=args.write_folder
ROOT_PATH_JSON_KG=args.json_kg_folder
...
def copy_allen_annotation(user, system, conversation_nel, i):
    if ALLEN_ES_LINKS in conversation_nel[2 * i].keys():
        user[ALLEN_TYPE_LINKS] = conversation_nel[2 * i][ALLEN_TYPE_LINKS]
        user[ALLEN_ES_LINKS] = conversation_nel[2 * i][ALLEN_ES_LINKS]
        user[ALLEN_SPANS] = conversation_nel[2 * i][ALLEN_SPANS]
        user[ALLEN_TAGS] = conversation_nel[2 * i][ALLEN_TAGS]
        if 'context' in user.keys():
            del user['context']
    if ALLEN_ES_LINKS in conversation[2 * i + 1].keys():
        system[ALLEN_TYPE_LINKS] = conversation_nel[2 * i + 1][ALLEN_TYPE_LINKS]
        system[ALLEN_ES_LINKS] = conversation_nel[2 * i + 1][ALLEN_ES_LINKS]
        system[ALLEN_SPANS] = conversation_nel[2 * i + 1][ALLEN_SPANS]
...
cache_file='cache/subgraph_cache.json'
filesnotfound = []
...
for sp in splits:
    ...
    for e, f in enumerate(files):
        ...
        if args.allennlpNER_folder:
            try:
                f_nel_allen = f'{args.allennlpNER_folder}/{sp}{f.split(sp)[1]}.jointlinking'
            except FileNotFoundError:
                print('File not found', sp)
                filesnotfound.append(sp)
                continue
        ...
        with open(f) as json_file:
                try:
                    ...
                    if not os.path.isdir(os.path.join(DST_ROOT_PATH, sp, dirName)):
                        os.makedirs(os.path.join(DST_ROOT_PATH, sp, dirName))
