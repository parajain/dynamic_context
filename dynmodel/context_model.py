...
class DynGraphContextModel(nn.Module):
    def __init__(self, args, device, gobal_syntax_vocabulary, checkpoint=None):
        super(DynGraphContextModel, self).__init__()
        self.args = args
        self.device = device
    ...
    def mean_pooling(self, model_output, attention_mask):
        token_embeddings = model_output # With custom bert
        input_mask_expanded = attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float()
        sum_embeddings = torch.sum(token_embeddings * input_mask_expanded, 1)
        sum_mask = torch.clamp(input_mask_expanded.sum(1), min=1e-9)
        return sum_embeddings / sum_mask
    
    def pad_and_encode_logical_form(self, logical_form_token_list, nodelinks, gobal_syntax_vocabulary):
            ...
            for iteridx, (toktype, tok) in enumerate(lftokens):
                if toktype in [ENTITY, RELATION, TYPE]:
                    ...
                    if tok in links:
                        ...
                        lf_vocab_encodings.append(links.index('MISSINGNODE') + global_vocab_len)
    ...
    def forward(self, batch, logger):
        ...
        lf_selected_indices = batch['lf_selected_indices'] 
        ...
        decoder_outputs, state = self.decoder(tgt[:, :-1], input_embeddings, dec_state, False, memory_masks=None, dynEmbed=graph_emb)
        decoder_outputs = self.generator({0: decoder_outputs, 1: graph_emb})

        return decoder_outputs, tgt
    ...
    def predict_greedy(self, input_ids, input_embeddings, graph_emb, batch):
        max_length=500
        with_cache=True        
        ...
        decoder_input = torch.full([1, 1], self.start_token, dtype=torch.long, device=device)
        results = {}
        results['batch'] = batch
        ...
        for dstep in range(max_length):
            ...
            dec_out, dec_states = self.model.decoder(decoder_input, input_embeddings, dec_states,
                                                        False,
